...
class Generator:
    ...
    def generate_ours(self, input_tensors, target_index, config, index=None, use_lrp=True, normalize_self_attention=True, apply_self_in_rule_10=True):
        ...
        # print(self.model.transformer.decoder.layers[0].self_attn.get_attn())
        # outputs = outputs['pred_logits']
        kwargs = {"alpha": 1,
                  "target_index": target_index}

        if index == None:
            # print("max:",outputs[0, target_index, :-1].max(-1))
        ...
        # initialize relevancy matrices
        # image_bboxes = encoder_blocks[0].self_attn.get_attn().shape[-1] ## 601*256
        # print("image_bboxes",image_bboxes)
        # queries_num = decoder_blocks[0].self_attn.get_attn().shape[-1]
        image_bboxes = config.max_node + 1
        queries_num = config.max_position_embeddings
        # image self attention matrix
        self.R_i_i = torch.eye(image_bboxes, image_bboxes).to(config.device)
        # queries self attention matrix
        self.R_q_q = torch.eye(queries_num, queries_num).to(config.device)
        # impact of image boxes on queries
        self.R_q_i = torch.zeros(queries_num, image_bboxes).to(config.device)
